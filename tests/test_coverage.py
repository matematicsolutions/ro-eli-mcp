...
from ro_eli_mcp.coverage import build_coverage
from ro_eli_mcp.server import ro_coverage


def _coverage():
    fn = getattr(ro_coverage, "fn", ro_coverage)
...
def test_tool_writes_an_audit_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("RO_ELI_AUDIT_DIR", str(tmp_path))
    _coverage()
    lines = [
        line
    ...
    assert any("ro_coverage" in line for line in lines), (
        "an audit entry was written but it does not name ro_coverage"
    )
