...
SOURCE = 'legislatie.just.ro (Portal Legislativ SOAP API)'
...
_FAMILIES: list[dict] = [{'name': 'National legislation', 'tool': 'ro_search / ro_get_act / ro_get_text'}]

_GAPS: list[dict] = [{'id': 'RO-001', 'family': 'National legislation', 'missing': 'Romania has no data.europa.eu ELI for this portal; eli_uri carries the canonical legislatie.just.ro URL.', 'fallback': 'Treat eli_uri as the national portal identifier.'}, {'id': 'RO-002', 'family': 'National legislation', 'missing': 'Acts of the same number exist across types (LEGE, DECRET, HOTARARE and others), so a bare number is ambiguous.', 'fallback': 'Always pair the number with its act type.'}, {'id': 'RO-003', 'family': 'National legislation', 'missing': 'Legal force on a given date is not evaluated. Text is returned as the source publishes it; this connector does not decide whether a provision is in force.', 'fallback': 'Check the validity or consolidation dates on the source record itself.'}]
